...
class MethodChannelSensors extends SensorsPlatform {
  ...
  static const EventChannel _heartRateEventChannel =
      EventChannel('dev.fluttercommunity.plus/sensors/heartrate');
  ...
  Stream<HeartRateEvent>? _heartRateEvents;
  ...
  Stream<HeartRateEvent> heartRateEventStream({
    ...
    _methodChannel.invokeMethod('setHeartRateSamplingPeriod', microseconds);
    _heartRateEvents ??=
        _heartRateEventChannel.receiveBroadcastStream().map((dynamic event) {
      final list = event.cast<double>();
      return HeartRateEvent(list[0]!);
    });
    return _heartRateEvents!;
  }
}
